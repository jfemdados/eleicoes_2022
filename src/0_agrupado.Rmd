...
shp_secoes_bairros <- shp_tse_secoes %>% 
  st_join(shp_bairros_pjf, join = st_covered_by)
...
shp_bairros_pjf %>% 
  ...
  #tm_shape(shp_tse_secoes) +
  #tm_dots(col = "red") +
  tm_shape(shp_secoes_bairros %>% filter(is.na(code_neighborhood))) +
  tm_dots(col = "blue")
```
...
shp_presidente_valid_tally_bairros <- shp_bairros_pjf %>% 
  select(name_neighborhood, pop_20, rend_pc, prop_fam_perfil_cad) %>% 
  st_join(shp_presidente_valid_tally, join = st_covers) %>% 
